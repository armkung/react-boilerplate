...
exports.config = {
  ...
  afterStep: function afterStep(stepResult) {
    if (stepResult.status != 'passed') {
      ...
      const featureName = stepResult.step.scenario.uri
        .replace(process.cwd(), '')
        .split('/')
        .slice(-1)
        .join('')
      ...
      const fileName = `${path}${featureName}_${browser.options.desiredCapabilities.browserName}_${new Date().toISOString().replace('.', ':')}.png`
